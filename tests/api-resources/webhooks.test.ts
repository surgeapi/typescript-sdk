...
describe('resource webhooks', () => {
  test.skip('unwrap', () => {
    ...
    const payload = '{"account_id":"acct_01japd271aeatb7txrzr2xj8sg","data":{"id":"call_01jjnn7s0zfx5tdcsxjfy93et2","contact":{"id":"ctc_01ja88cboqffhswjx8zbak3ykk","phone_number":"+18015551234","email":"dom@toretto.family","first_name":"Dominic","last_name":"Toretto","metadata":{"car":"1970 Dodge Charger R/T"}},"duration":184,"initiated_at":"2025-03-31T21:01:37Z","status":"completed"},"timestamp":"2024-10-21T23:29:41Z","type":"call.ended"}';
    const msgID = '1';
    ...
    client.withOptions({webhookSigningSecret: key}).webhooks.unwrap(payload, { headers })
    client.withOptions({webhookSigningSecret: 'whsec_aaaaaaaaaa=='}).webhooks.unwrap(payload, { headers, key })
    expect(() => {
    ...
    expect(() => {
      const wrongKey = 'whsec_aaaaaaaaaa==';
      client.withOptions({webhookSigningSecret: wrongKey}).webhooks.unwrap(payload, { headers })
    }).toThrow('No matching signature found');
    expect(() => {
    ...
    expect(() => {
      const badSig = wh.sign(msgID, timestamp, 'some other payload');
      client.withOptions({webhookSigningSecret: key}).webhooks.unwrap(payload, { headers: {...headers, 'webhook-signature': badSig }})
    }).toThrow('No matching signature found');
    expect(() => {
      client.webhooks.unwrap(payload, { headers: {...headers, 'webhook-timestamp': '5' }, key})
    }).toThrow('Message timestamp too old');
    expect(() => {
      client.withOptions({webhookSigningSecret: key}).webhooks.unwrap(payload, { headers: {...headers, 'webhook-timestamp': '5' }})
    }).toThrow('Message timestamp too old');
    expect(() => {
      client.webhooks.unwrap(payload, { headers: {...headers, 'webhook-id': 'wrong' }, key})
    }).toThrow('No matching signature found');
    expect(() => {
      client.withOptions({webhookSigningSecret: key}).webhooks.unwrap(payload, { headers: {...headers, 'webhook-id': 'wrong' }})
    }).toThrow('No matching signature found');
  });
});
