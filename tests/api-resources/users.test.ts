...
describe('resource users', () => {
  ...
  test.skip('users: only required params', async () => {
    const responsePromise = client.users.users('acct_01j9a43avnfqzbjfch6pygv1td', { first_name: 'Brian' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('users: required and optional params', async () => {
    const response = await client.users.users('acct_01j9a43avnfqzbjfch6pygv1td', {
      first_name: 'Brian',
      last_name: "O'Conner",
      metadata: { email: 'boconner@toretti.family', user_id: 'string' },
      photo_url: 'https://toretti.family/people/brian.jpg',
